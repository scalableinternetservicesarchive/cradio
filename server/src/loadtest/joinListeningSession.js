...
export function setup() {
  ...
  const nameGen =
    '{"operationName":"CreatePartyRocker","variables":{"input":{"name":"SessionOwner"}},"query":"mutation CreatePartyRocker($input: PartyRockerInfo!) { \\n createPartyRocker(input: $input) { \\n id }}"}'
  const mainPartyRocker = http.post('http://localhost:3000/graphql', nameGen, {
    headers: {
...
export default function (data) {
  ...
  const newPartyRocker = http.post('http://localhost:3000/graphql', nameGen, {
    headers: {
      'Content-Type': 'application/json',
    },
  })
  ...
  const sessGen =
    '{"operationName":"JoinListeningSession","variables":{"input":{"partyRockerId":' +
    String(createdRockerId) +
    ', "sessionId":' +
    String(useSessionId) +
    '}},"query":"mutation JoinListeningSession($input: JoinSessionInfo!) {\\n joinListeningSession(input: $input)}"}'
  const joinSession = http.post('http://localhost:3000/graphql', sessGen, {
    headers: {
      'Content-Type': 'application/json',
    },
  })
...
export function teardown(data) {
  ...
  const deleteSessionResult = http.post(
    ...
    '{"operationName":"DeleteListeningSession","variables":{"sessionId":' +
      deleteId +
      '},"query":"mutation DeleteListeningSession($sessionId: Int!) { \\n deleteListeningSession(sessionId: $sessionId)}"}',
    {
      headers: {
        'Content-Type': 'application/json',
      },
