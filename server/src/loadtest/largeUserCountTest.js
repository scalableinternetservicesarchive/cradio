...
export function setup() {
  ...
  for (let i = 1; i < totalEntries + 1; i++) {
    const mainPartyRocker = http.post(
      ...
      '{"operationName":"CreatePartyRocker","variables":{"input":{"name":"Rocker ' +
        i +
        '"}},"query":"mutation CreatePartyRocker($input: PartyRockerInfo!) { \\n createPartyRocker(input: $input) { \\n id }}"}',
      {
        headers: {
...
export default function (data) {
  ...
  const sessGen =
    '{"operationName":"CreateListeningSession","variables":{"partyRockerId":' +
    data.partyRockerIdArray[partyRockerIndex] +
    '},"query":"mutation CreateListeningSession($partyRockerId: Int!) {\\n createListeningSession(partyRockerId: $partyRockerId) { \\n id timeCreated }}"}'
  const mainSession = http.post('http://localhost:3000/graphql', sessGen, {
    headers: {
      'Content-Type': 'application/json',
    },
  })
  ...
  if (mainSession.body.length == 0) {
    ...
    const queueGen =
      '{"operationName":"AddToQueue","variables":{"input":{"songId":' +
      String(Math.floor(Math.random() * 5 + 1)) +
      ', "listeningSessionId":' +
      String(listeningSessionID) +
      '}},"query":"  mutation AddToQueue($input: QueueInfo!) { \\n addToQueue(input: $input) \\n}"}'
    const addToQueueWorked = http.post('http://localhost:3000/graphql', queueGen, {
      headers: {
        'Content-Type': 'application/json',
      },
    })
...
export function teardown(data) {
  ...
  //     'http://localhost:3000/graphql',
  //     '{"operationName":"DeletePartyRocker","variables":{"partyRockerId":' +
  //       printId +
  //       '},"query":"mutation DeletePartyRocker($partyRockerId: Int!) { \\n deletePartyRocker(partyRockerId: $partyRockerId)}"}',
  //     {
  //       headers: {
  //         'Content-Type': 'application/json',
  //       },
  ...
  const numListeningSessions = http.get('http://localhost:3000/numSessions')
  //console.log('num listneing sessions: ', numListeningSessions.body)
  const numNumNumListeningSessions = Number(numListeningSessions.body)
  //Note, some of these calls will fail because of how our schema works but thats ok it will still delete all that were created
  //its becaise we are deleting all listening sessions that were possibly created even before this test. (numListeningsessions does not ever decrement)
  for (let i = 1; i <= numNumNumListeningSessions; i++) {
    ...
    // console.log('delete listneing sesion result :       ', deleteSessionResult.body)
  }
}
